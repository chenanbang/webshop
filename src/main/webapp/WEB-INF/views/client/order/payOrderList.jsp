...
<head>
	...
	<script type="text/javascript">
		...
		function deleteOrder(orderId){
			...
			targetForm.action="${pageContext.request.contextPath }/order/delete/pay/"+orderId;
			targetForm.submit();
		}
				...
				<div class="main clearfix" style="border: none; width: 1100px; margin: 0px auto; margin-top: 20px;">
					<table style="text-align: center; width:1100px; margin: 0px auto;" id="tb">
						...
						<tbody>
							...
							<tr>
								<td colspan="9">
									<a class="a" href="${pageContext.request.contextPath }/order/listIsOrNotPayOrdersByUserId/1/1">首页</a>
									<c:if test="${pageBean.pageStartNum > 1}">
										<a class="a" href="${pageContext.request.contextPath }/order/listIsOrNotPayOrdersByUserId/${pageBean.pageStartNum-1 }/1">上一页</a>
									</c:if>
									<%
										...
										for(int i=begin;i<=end;i++){
											if(i==currentPageNum){
												...
												%><a href="${pageContext.request.contextPath }/order/listIsOrNotPayOrdersByUserId/<%=i %>/1" class="pagingStyle"><%=i %></a><%
											}
										}
									%>
									<c:if test="${pageBean.totalPage > pageBean.pageStartNum}">
										<a class="a" href="${pageContext.request.contextPath }/order/listIsOrNotPayOrdersByUserId/${pageBean.pageStartNum+1 }/1">下一页</a>
									</c:if>
									
									<c:if test="${pageBean.totalPage==0 }">
										<a class="a" href="${pageContext.request.contextPath }/order/listIsOrNotPayOrdersByUserId/${pageBean.totalPage+1 }/1">尾页</a>
									</c:if>
									
									<c:if test="${pageBean.totalPage!=0 }">
										<a class="a" href="${pageContext.request.contextPath }/order/listIsOrNotPayOrdersByUserId/${pageBean.totalPage }/1">尾页</a>
									</c:if>
								</td>
							</tr>
						</tbody>
