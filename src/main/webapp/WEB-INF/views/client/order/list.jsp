...
<head>
	...
	<script type="text/javascript">
		...
		function deleteOrder(orderId){
			...
			targetForm.action="${pageContext.request.contextPath }/order/delete/all/"+orderId;
			targetForm.submit();
		}
					...
					<table style="text-align: center; width:1100px; margin: 0px auto;" id="tb">
						<thead>
							<tr style="border: none; ">
								<td colspan="9" style="border-right: none;padding-bottom: 9px; text-align: center; font-size: 16px;"><strong>订 单 管 理 中 心</strong></td>
							</tr>
						...
						<tbody>
							...
							<tr>
								<td colspan="9">
									<a class="a" href="${pageContext.request.contextPath }/order/listOrdersByUserId/1">首页</a>
									<c:if test="${pageBean.pageStartNum > 1}">
										<a class="a" href="${pageContext.request.contextPath }/order/listOrdersByUserId/${pageBean.pageStartNum-1 }">上一页</a>
									</c:if>
									<%
										...
										for(int i=begin;i<=end;i++){
											if(i==currentPageNum){
												...
												%><a href="${pageContext.request.contextPath }/order/listOrdersByUserId/<%=i %>" class="pagingStyle"><%=i %></a><%
											}
										}
									%>
									<c:if test="${pageBean.totalPage > pageBean.pageStartNum}">
										<a class="a" href="${pageContext.request.contextPath }/order/listOrdersByUserId/${pageBean.pageStartNum+1 }">下一页</a>
									</c:if>
									
									<c:if test="${pageBean.totalPage==0 }">
										<a class="a" href="${pageContext.request.contextPath }/order/listOrdersByUserId/${pageBean.totalPage+1 }">尾页</a>
									</c:if>
									
									<c:if test="${pageBean.totalPage!=0 }">
										<a class="a" href="${pageContext.request.contextPath }/order/listOrdersByUserId/${pageBean.totalPage }">尾页</a>
									</c:if>
								</td>
							</tr>
						</tbody>
