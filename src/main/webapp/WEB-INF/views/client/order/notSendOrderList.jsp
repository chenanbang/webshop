...
<head>
	...
	<script type="text/javascript">
		...
		function deleteOrder(orderId){
			...
			targetForm.action="${pageContext.request.contextPath }/order/delete/notSend/"+orderId;
			targetForm.submit();
		}
				...
				<div class="main clearfix" style="border: none; width: 1100px; margin: 0px auto; margin-top: 20px;">
					<table style="text-align: center; width:1100px; margin: 0px auto;" id="tb">
						...
						<tbody>
							...
							<tr>
								<td colspan="9">
									<a class="a" href="${pageContext.request.contextPath }/order/listIsOrNotSendOrdersByUserId/1/0">首页</a>
									<c:if test="${pageBean.pageStartNum > 1}">
										<a class="a" href="${pageContext.request.contextPath }/order/listIsOrNotSendOrdersByUserId/${pageBean.pageStartNum-1 }/0">上一页</a>
									</c:if>
									<%
										...
										for(int i=begin;i<=end;i++){
											if(i==currentPageNum){
												...
												%><a href="${pageContext.request.contextPath }/order/listIsOrNotSendOrdersByUserId/<%=i %>/0" class="pagingStyle"><%=i %></a><%
											}
										}
									%>
									<c:if test="${pageBean.totalPage > pageBean.pageStartNum}">
										<a class="a" href="${pageContext.request.contextPath }/order/listIsOrNotSendOrdersByUserId/${pageBean.pageStartNum+1 }/0">下一页</a>
									</c:if>
									
									<c:if test="${pageBean.totalPage==0 }">
										<a class="a" href="${pageContext.request.contextPath }/order/listIsOrNotSendOrdersByUserId/${pageBean.totalPage+1 }/0">尾页</a>
									</c:if>
									
									<c:if test="${pageBean.totalPage!=0 }">
										<a class="a" href="${pageContext.request.contextPath }/order/listIsOrNotSendOrdersByUserId/${pageBean.totalPage }/0">尾页</a>
									</c:if>
								</td>
							</tr>
						</tbody>
